...
- **Compuerta legal (RGPD, NO revertir)**: `crear_sesion` llama a `_exigir_consentimiento_y_dpa` — no se abre una sesión real sin el **DPA** del centro (`DocumentoLegal` tipo `dpa`) y el **consentimiento** de cada participante (`Consentimiento`). El demo/vitrina no pasa por aquí. El seed da DPA+consentimientos al centro demo.
- **Suscripción/acceso**: `Centro.estado_suscripcion` (prueba/activa/cortesia/suspendido/cancelada) + `fecha_fin_prueba`. `deps._exigir_centro_activo` corta el acceso (403) si suspendido/cancelada o prueba caducada. Cobro por **Stripe** (`routers/facturacion.py`: checkout con tramos 125 €/30 + 3 €/extra, cantidad=personas activas; webhook activa/suspende). Superadmin: `PATCH /plataforma/centros/{id}/suscripcion` (cortesía/suspender/+días).
- **Puesta en marcha**: `GET /puesta-en-marcha` (`routers/onboarding.py`) da el estado real del centro para el asistente del panel (`/puesta-en-marcha`).
- **Emparejado por QR**: el panel (Tablets) muestra un QR del código; la app lo escanea (`mobile_scanner`, `escanear_qr_screen.dart`). APK público en GitHub Releases (`releases/latest/download/Trazo.apk`); página de instalación `apps/landing/instalar.html`. Para simular tablets sin hardware: `trazo-tablet.pages.dev` (web, CORS permitido).
