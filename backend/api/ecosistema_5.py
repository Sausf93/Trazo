...
async def main():
    ...
    async with AsyncClient(transport=transport, base_url="http://test", timeout=60) as c:
        ...
        for cn in ("Centro Norte", "Centro Sur", "Centro Este", "Centro Oeste", "Centro Central"):
            ...
            for i in range(2):
                ...
                facils.append(h)
            check(len(facils) == 2, "%s: 2 integradoras creadas" % cn)
        ...
        print("== 2 grupos de 10 SIMULTÁNEOS por centro (10 salas vivas a la vez) ==")
        for cn, d in centros.items():
            grupos = [d["usuarios"][i * 10:(i + 1) * 10] for i in range(2)]
            ...
            check(esperadas <= abiertas and len(esperadas) == 2,
                  "%s: 2 salas conviven abiertas a la vez (%d)" % (cn, len(esperadas & abiertas)))
            ...
            ok_kiosco = len(salas) == 2 and all(por_sala.get(sid) == set(g) for sid, g, _ in d["salas"])
            check(ok_kiosco, "%s: el kiosco ve las 2 salas, cada una con los suyos" % cn)
        print()
        ...
        print("== Panel en vivo: cada grupo muestra sus 10 fichas ==")
        for cn, d in centros.items():
            todas_ok = True
            for sid, grupo, _ in d["salas"]:
                l = await c.get("/sesiones/%s/live" % sid, headers=d["coord"])
                if l.status_code != 200 or len(l.json()["fichas"]) != 10:
                    todas_ok = False
            check(todas_ok, "%s: las 2 salas muestran 10 fichas en vivo" % cn)
        print()
        ...
        print("== Aislamiento entre los 5 centros (matriz completa) ==")
        ...
        check(aisl_ok, "cada centro NO ve nada de los demás (20 pares) %s" % ("" if aisl_ok else detalles))
        ...
        for cn, d in centros.items():
            ...
            check(all(mias), "%s: cada integradora recupera SU sala (2/2)" % cn)

    app.dependency_overrides.clear()
    await eng.dispose()
    print("\n==== RESULTADO ECOSISTEMA 5 CENTROS: %d OK, %d FAIL ====" % (len(oks), len(fails)))
    if fails:
        print("FALLOS:")
        for f in fails:
            print("  - " + f)
