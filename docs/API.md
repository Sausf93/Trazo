...
| POST | `/sesiones` | Crear/abrir sala. Body: `tipo`, `nombre?`, `modo?`, `ejercicio_compartido_id?`, `participantes` (ids), y `configs?` = config por participante para ESTA sesión: `[{usuario_final_id, nivel?, lineas:[{bloque, n}]}]` (`n` entre 1 y 50). Un participante con config (con líneas) saca su cola de ahí; si solo trae `nivel` sin líneas, o no trae config, usa su plan. **Programar**: con `programar:true` (+ `programada_para?` = fecha `YYYY-MM-DD`) la sala se guarda como PROGRAMADA (borrador para otro día): no se abre a los kioscos hasta `/abrir`. Los participantes deben ser del centro del staff (403 si no). |
| GET | `/sesiones/programadas?centro_id=` | Salas dejadas preparadas (sin abrir) del centro, cada una con sus participantes y config fijada, ordenadas por `programada_para`. Para que la maestra las repase y abra. |
| PATCH | `/sesiones/{id}/abrir` | Abre una sesión PROGRAMADA: pasa a estar "en vivo" (`abierta=true`) y se convierte en la sesión activa del centro; luego se reparten tablets y se pulsa `/iniciar`. |
| PUT | `/sesiones/{id}/config` | Edita una sesión PROGRAMADA antes del día (reemplaza `nombre?`, `modo?`, `programada_para?`, `participantes`, `configs`). Solo si aún no está abierta ni cerrada (409 en caso contrario). |
| DELETE | `/sesiones/{id}` | Descarta una sesión PROGRAMADA sin abrir (204). 409 si ya se abrió o inició. |
| GET | `/sesiones/activa?centro_id=` | Sesión ABIERTA más reciente del centro + participantes (la tablet participante la usa para descubrir la sala). Las programadas no aparecen aquí hasta abrirlas. Devuelve `nombre`, `iniciada`, `sesion_id=null` si no hay. |
